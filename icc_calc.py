...
def calculate_metrics(**kwargs) -> Dict[str, Any]:
    """
    Core domain algorithm for intraclass-correlation-icc-calc.
    """
    params = {}
    ...
    return {
        "tool": "intraclass-correlation-icc-calc",
        "score": rounded_score,
        "classification": tier,
...
def process_batch(input_csv: str, output_csv: str) -> None:
    ...
    if not input_path.is_file():
        raise ValueError(f"Input path is not a regular file: {input_csv}")
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="Intraclass Correlation Coefficient (ICC) & Design Effect")
